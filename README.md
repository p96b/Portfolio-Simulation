...
For the sake of testing all available functions of our program, we provided a pre-existing portfolio. Certain functions, such as the historical performance line chart only make sense if the portfolio was held for some time (i.e., there are already data points available). In simple terms, we want to avoid that the program must be used for several days in order to see all available functions. However, the portfolio can be reset at any time using the function in the main menu if you want to start form zero. This function wipes the CSV files and deletes the cash.pk file, which will then be re-created once the user starts the code again. 
